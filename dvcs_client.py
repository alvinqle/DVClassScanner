...
phone_path = os.path.join(directory, './creds/phone_numbers.txt')
phone_numbers = list(line.strip() for line in open(phone_path))
    ...
    def check_availability(self, recipient_number, campus, term, course):
        try:
            ...
            campus_option = WebDriverWait(driver, 10).until(
                expected_conditions.presence_of_element_located((By.XPATH, "//input[@value='{0}']".format(campus))))
            ...
            course_name_td = WebDriverWait(driver, 10).until(expected_conditions.presence_of_all_elements_located(
                (By.XPATH, "//span[contains(text(), '{0}')]".format(course))))
